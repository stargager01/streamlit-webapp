    ...
    with st.container(border=True):
        ...
        st.text_area(
            ...
            on_change=lambda: st.session_state.update({
                "habit_other": st.session_state["habit_other_widget"]
            }),
    ...
    with st.container(border=True):
        ...
        st.radio(
            ...
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_deviation", "선택 안 함")
            ),
            key="jaw_deviation_widget",
            on_change=lambda: st.session_state.update({
                "jaw_deviation": st.session_state["jaw_deviation_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_instability", "선택 안 함")
            ),
            key="jaw_instability_widget",
            on_change=lambda: st.session_state.update({
                "jaw_instability": st.session_state["jaw_instability_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_opening_pain", "선택 안 함")
            ),
            key="jaw_opening_pain_widget",
            on_change=lambda: st.session_state.update({
                "jaw_opening_pain": st.session_state["jaw_opening_pain_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_opening_sound", "선택 안 함")
            ),
            key="jaw_opening_sound_widget",
            on_change=lambda: st.session_state.update({
                "jaw_opening_sound": st.session_state["jaw_opening_sound_widget"]
            })
        )
...
def render_step_9_rom_observations_2():
    ...
    with st.container(border=True):
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_closing_deviation", "선택 안 함")
            ),
            key="jaw_closing_deviation_widget",
            on_change=lambda: st.session_state.update({
                "jaw_closing_deviation": st.session_state["jaw_closing_deviation_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_closing_instability", "선택 안 함")
            ),
            key="jaw_closing_instability_widget",
            on_change=lambda: st.session_state.update({
                "jaw_closing_instability": st.session_state["jaw_closing_instability_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_closing_pain", "선택 안 함")
            ),
            key="jaw_closing_pain_widget",
            on_change=lambda: st.session_state.update({
                "jaw_closing_pain": st.session_state["jaw_closing_pain_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_closing_sound", "선택 안 함")
            ),
            key="jaw_closing_sound_widget",
            on_change=lambda: st.session_state.update({
                "jaw_closing_sound": st.session_state["jaw_closing_sound_widget"]
            })
        )
...
def render_step_10_rom_observations_3():
    ...
    with st.container(border=True):
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_lateral_asymmetry", "선택 안 함")
            ),
            key="jaw_lateral_asymmetry_widget",
            on_change=lambda: st.session_state.update({
                "jaw_lateral_asymmetry": st.session_state["jaw_lateral_asymmetry_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_lateral_pain", "선택 안 함")
            ),
            key="jaw_lateral_pain_widget",
            on_change=lambda: st.session_state.update({
                "jaw_lateral_pain": st.session_state["jaw_lateral_pain_widget"]
            })
        )
        ...
        st.radio(
            ...
            options=["예", "아니오", "선택 안 함"],
            index=["예", "아니오", "선택 안 함"].index(
                st.session_state.get("jaw_lateral_sound", "선택 안 함")
            ),
            key="jaw_lateral_sound_widget",
            on_change=lambda: st.session_state.update({
                "jaw_lateral_sound": st.session_state["jaw_lateral_sound_widget"]
            })
        )
...
def render_step_11_palpation():
    ...
    with st.container(border=True):
        ...
        st.text_area(
            ...
            on_change=lambda: st.session_state.update({
                "palpation_other": st.session_state["palpation_other_widget"]
            }),
